"""Track B PILOT, step 3: score the real routing_node.py/escalation_node.py
against data/routing_ground_truth_99.csv (built by merge_routing_annotations_99.py).

Two runs per the evaluation proposal (DATA_SCIENCE_EVALUATION_PROPOSAL.md
Sec7.2), so classifier mistakes and routing-rule mistakes don't get mixed
together:

1. LIVE run - the real local classifier (classify_ticket, one call per
   query) supplies category/priority/sentiment/confidence exactly as
   production would, then decide_routing() and decide_escalation() are
   called on that real output. This is the realistic number, but a wrong
   answer here could come from the classifier OR the routing rules.
2. ROUTING-LOGIC-ONLY run - decide_routing() is called again on the SAME
   query, but with category replaced by the human-verified ground truth
   domain and confidence forced to 0.99 (safely above the 0.7 cutoff, so
   the low-confidence "both" rule can't fire by accident). priority/
   sentiment are reused from the same classify_ticket() call (not
   part of what this run isolates - only category is being replaced).
   This removes classifier error, so what's left belongs to the routing/
   escalation rules themselves.

Only defined for rows whose routing_ground_truth is technical/billing/hr
(a real domain string decide_routing can be handed directly) - "both" and
"escalation" aren't valid category inputs, so those rows are counted in
the LIVE metrics but skipped from ROUTING-LOGIC-ONLY ones (skip count
reported, not silently dropped).

Escalation is scored against only the 5 triggers decide_escalation() can
fire from ticket-intrinsic signals (critical_priority, negative_sentiment,
hr_process_required, no_usable_routing_signal, low_confidence_dual_domain)
- an explicit scoping decision, not an oversight: the other 3 triggers
(dependency_failure, misroute_unresolved, reflection_cap_reached) only
fire after a real run through specialists + validation + reflection,
which this script doesn't execute. McNemar's test on the reroute retry
(also in the proposal) needs the same real run to produce a genuine
misroute event, so it's skipped here for the same reason - see
../README.md's limitations section.

Usage:
    python3 run_routing_evaluation_99.py
"""
...
GT_PATH = DATA_DIR / "routing_ground_truth_99.csv"
...
def escalation_prf(y_true: list[bool], y_pred: list[bool]) -> dict:
    ...
    return {
        ...
        "missed_escalations_fn": fn,  # worse mistake: a ticket that needed a human didn't get one
...
async def run() -> None:
    ...
    with open(RESULTS_DIR / "per_query_routing_results_99.csv", "w", newline="", encoding="utf-8") as f:
    ...
    with open(RESULTS_DIR / "routing_eval_summary_99.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, default=str)
    print(f"\nWrote {RESULTS_DIR / 'routing_eval_summary_99.json'} and per_query_routing_results_99.csv")
