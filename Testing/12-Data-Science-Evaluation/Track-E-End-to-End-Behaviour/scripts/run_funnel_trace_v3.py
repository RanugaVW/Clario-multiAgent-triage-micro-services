"""Track E, step 1 (v3) - verifies the real code fix made after run_funnel_trace_v2.py's
paired comparison found that reflection did not reliably improve the judge
score (5 improved, 16 unchanged, 8 got worse, out of 29 reflected tickets).

The actual fix now lives in the real pipeline, not in this script:
reflection_node.py snapshots each domain's first draft, and
response_judge_node.py now scores both the original and the rewritten draft
and keeps whichever one the judge actually prefers, falling back to the
original when reflection didn't help. See both files' docstrings for the
production-side change.

This script still independently re-scores the pre-reflection snapshot itself
(same wrapper technique as v2) - not because the pipeline needs it anymore,
but as an external check: the pipeline's own final_judge_score_mean should
now equal max(pre-reflection score, rewrite score) for every reflected
ticket, and comparing this script's own independent re-score against that
final number is how that claim gets verified rather than assumed.
...
async def run(start: int, limit: int | None) -> None:
    ...
    out_path = RESULTS_DIR / "funnel_trace_v3.csv"
    fieldnames = [
