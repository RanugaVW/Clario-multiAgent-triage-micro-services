"""Track E, step 4 (v6) - checks whether expanding the KB (5 new documents +
1 fix to an existing one, covering topics the Track A ground-truth annotator
explicitly flagged as missing: WebXpay/checkout discrepancies, manual
payment-proof status, payment/enrollment misclassification, refund
eligibility, account-access edge cases beyond password reset, and enrollment-
sync mismatches) measurably changes Track E's live funnel numbers on the
same 70 tickets used throughout this track.

Same 70 tickets, same pipeline code as v5 - the only thing that changed
between v5 and this run is the KB content and its rebuilt vector index. Any
difference in grounding-related failures (failure_type=quality/policy) or
escalation rate between v5 and v6 is attributable to the KB change, not to
code or sampling drift alone (though the usual run-to-run LLM/judge sampling
noise documented in Section 2 still applies).

Usage:
    python3 run_funnel_trace_v6.py [--start N] [--limit N]
"""
...
async def run(start: int, limit: int | None) -> None:
    ...
    out_path = RESULTS_DIR / "funnel_trace_v6.csv"
    fieldnames = [
