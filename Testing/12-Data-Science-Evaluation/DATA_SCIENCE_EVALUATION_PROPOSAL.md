...
**Status: complete.** This exercise has been run in full. 71 real tickets were reviewed; one (ticket #20, a plain duplicate of another ticket with no content of its own) was skipped by both annotators, leaving 70. **Corrected 2026-09-08** (the originally-used annotator files turned out to be the wrong version — see `Track-A-Retrieval-Quality/TEST_REPORT_V2.md` §0): the two annotators' domain and document sets overlapped on 60 of 70 (85.7%) before any discussion, matched exactly on 47 of 70 (67.1%); every non-exact row was resolved by taking the union of both answers. Full results and all charts are in `Track-A-Retrieval-Quality/TEST_REPORT_V2.md`.
...
- Tracks A, B, C, D, and E all run on one 70-ticket sample. This is real, high-quality data (85.7% two-person agreement; see the correction in `Track-A-Retrieval-Quality/TEST_REPORT_V2.md` §0), but it is a small sample overall — HR-specific findings, including Track A's strong HR numbers, should be read as an early look, not a fully powered result.
