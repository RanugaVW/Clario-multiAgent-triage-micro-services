...
async def ws_transcribe(websocket: WebSocket):
    ...
    # First fatal decode error, reported to the client instead of a final
    # transcript. A broken model fails identically on every chunk, so retrying
    # only floods the log and delays the error the user actually needs.
    ...
    try:
        ...
        while True:
            ...
            if message.get("type") == "websocket.disconnect":
                return
            # The decoder gave up (see ``decode_error``); stop reading audio.
            if stop.is_set():
                break
        ...
        if decode_error:
            await websocket.send_json(
                {
                    "type": "error",
                    "message": f"Transcription failed: {decode_error[0]}",
                }
            )
        else:
