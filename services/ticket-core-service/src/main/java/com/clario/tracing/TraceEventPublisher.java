 ...
 * enabled, publish() dispatches via CompletableFuture.runAsync (matching
 * the existing async-dispatch pattern already used in TicketService for
 * the Redis enqueue) so it never blocks the caller's own thread, and any
 * exception inside the async task is caught and logged, never rethrown.
 */
@Component
public class TraceEventPublisher {

    private static final Logger log = LoggerFactory.getLogger(TraceEventPublisher.class);
    private static final String SERVICE_NAME = "ticket-core-service";
