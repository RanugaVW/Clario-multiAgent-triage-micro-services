 ...
 * verification at all (a spoofable IDOR). It now independently verifies the
 * Supabase-issued JWT itself rather than relying solely on the gateway,
 * since this service is reachable directly on the Docker network (and on
 * its own port in local/dev runs) and must not assume every caller went
 * through the gateway's checks first.
 */
@Configuration
@EnableWebSecurity
