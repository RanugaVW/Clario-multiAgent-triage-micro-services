...
/**
 * Regression tests for the FR-002 / SEC-002 fix: ticket-core-service used to
 * trust a client-supplied X-User-Id header with no verification at all. It
 * now requires a signature-verified JWT and derives the caller's identity
 * exclusively from its "sub" claim.
 */
...
class TicketControllerSecurityTest {
    ...
    void createTicket_withValidToken_createsTicketForSubjectFromToken() throws Exception {
        ...
        when(ticketService.createTicket(any(), any(), any(), any(), any())).thenReturn(saved);

        mockMvc.perform(post("/api/tickets")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"rawText\":\"help, my login is broken\",\"subject\":\"Login issue\"}"))
                .andExpect(status().isAccepted());

        verify(ticketService).createTicket(
                org.mockito.ArgumentMatchers.eq("help, my login is broken"),
                org.mockito.ArgumentMatchers.eq("Login issue"),
    ...
    void createTicket_withBlankRawText_isRejectedWithDescriptiveError() throws Exception {
        ...
        mockMvc.perform(post("/api/tickets")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"rawText\":\"\",\"subject\":\"Login issue\"}"))
        ...
        verify(ticketService, org.mockito.Mockito.never()).createTicket(any(), any(), any(), any(), any());
    }
    ...
    void createTicket_whenDatabaseFails_returns503WithoutLeakingInternals() throws Exception {
        ...
        when(ticketService.createTicket(any(), any(), any(), any(), any()))
                .thenThrow(new org.springframework.dao.TransientDataAccessResourceException("connection refused: db.internal:5432"));
