...
class TraceEventPublisherTest {
    ...
    void disabled_publisher_never_touches_the_rest_template() {
        ...
        publisher.publish("t1", "c1", "persisted", "done", Map.of());

        verifyNoInteractions(restTemplate);
    }

    @Test
    void enabled_publisher_posts_the_expected_body() throws InterruptedException {
        RestTemplate restTemplate = mock(RestTemplate.class);
        TraceEventPublisher publisher = new TraceEventPublisher(restTemplate, true, "http://localhost:8700");

        publisher.publish("t1", "c1", "persisted", "done", Map.of("k", "v"));
        Thread.sleep(200); // publish() dispatches via CompletableFuture.runAsync - give it a moment
    ...
    void a_downstream_error_never_propagates_out_of_publish() {
        ...
        assertDoesNotThrow(() -> publisher.publish("t1", "c1", "persisted", "done", Map.of()));
    }
}
